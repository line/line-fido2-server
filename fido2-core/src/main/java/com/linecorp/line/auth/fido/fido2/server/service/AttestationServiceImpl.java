...
//            throw new FIDO2ServerRuntimeException(InternalErrorCode.USER_PRESENCE_FLAG_NOT_SET);
    ...
    private void verifyCertificateChainOfTrust(AttestationObject attestationObject, AttestationVerificationResult attestationVerificationResult, List<String> attestationRootCertificates) {
        try {
            ...
            boolean isTopCertMatched = isTopIntermediateCertificateSameWithRootCertificates(attestationVerificationResult, trustAnchors, attestationVerificationResult.getTrustPath().size());

            if (isSelfSignedAttestation(isTopCertMatched, attestationVerificationResult.getType()
            ...
            if (attestationObject.getFmt() == AttestationStatementFormatIdentifier.ANDROID_KEY) {
                AdditionalRevokeChecker.checkAndroidKeyAttestationRevokedCert(revokeCheckerClient, attestationVerificationResult.getTrustPath());
            }
    ...
    private boolean isSelfSignedAttestation(boolean isTopCertMatched, boolean isBasicAttestationType, boolean hasOnlyOneTrustPath) {
        return isTopCertMatched && isBasicAttestationType && hasOnlyOneTrustPath;
    }
}
